...
const editFinancialRecord = async (req, res) => {
  ...
  const calculateMonthlyTotals = async (req, res) => {
    try {
      ...
      const results = await FinancialModel.aggregate([
        ...
        {
          $group: {
            _id: { month: { $month: "$date" }, type: { $toLower: "$trimmedType" } },  // Use lowercase for consistent matching
  ...
  const calculateYearlyTotals = async (req, res) => {
    try {
      ...
      const expenses = results.find(r => r.type === "expense")?.totalAmount || 0;  // Compare lowercase 'expense'
